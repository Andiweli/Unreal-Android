...
#include "Core.h"
#ifndef UE1_DEBUG_TRAP
#if defined(_MSC_VER)
#define UE1_DEBUG_TRAP() __debugbreak()
#elif defined(__GNUC__) || defined(__clang__)
#define UE1_DEBUG_TRAP() __builtin_trap()
#else
#define UE1_DEBUG_TRAP() abort()
#endif
#endif
...
#ifdef PLATFORM_WIN32
static UBOOL UE1Win32HasPathSeparator( const char* Filename )
{
	return Filename && (strchr( Filename, '\\' ) || strchr( Filename, '/' ));
}

static UBOOL UE1Win32EndsWithDllExtension( const char* Filename )
{
	if( !Filename )
		return 0;

	const size_t Len = strlen( Filename );
	const size_t ExtLen = strlen( DLLEXT );

	return Len >= ExtLen && appStricmp( Filename + Len - ExtLen, DLLEXT ) == 0;
}

static const char* UE1Win32PackageNameNoExt( const char* Filename, char* Out, INT OutSize )
{
	check(Filename);
	check(Out);
	check(OutSize>0);
	...
	appStrncpy( Out, Base, OutSize );
	Out[OutSize-1] = 0;

	char* Dot = strrchr( Out, '.' );
	if( Dot && appStricmp( Dot, DLLEXT ) == 0 )
		*Dot = 0;

	return Out;
}

static UBOOL UE1Win32ShouldSkipDllBind( const char* Filename )
{
	char PackageName[256];
	UE1Win32PackageNameNoExt( Filename, PackageName, ARRAY_COUNT(PackageName) );

#if defined(PLATFORM_SDL) && defined(UE1_SDL_SKIP_LEGACY_WINDRV_DLLS)
	// SDL-Builds verwenden NSDLDrv. Alte originale Window.dll/WinDrv.dll
	// aus dem Spiel-Systemordner dÃ¼rfen nicht in den neuen Port hineinladen.
	if( appStricmp( PackageName, "Window" ) == 0 || appStricmp( PackageName, "WinDrv" ) == 0 )
		return 1;
#endif

#if defined(PLATFORM_SDL) && defined(UE1_SDL_SKIP_LEGACY_EDITOR_DLL)
	if( appStricmp( PackageName, "Editor" ) == 0 )
		return 1;
#endif

#if defined(PLATFORM_SDL) && defined(UE1_SDL_SKIP_LEGACY_SOFTDRV_DLL)
	if( appStricmp( PackageName, "SoftDrv" ) == 0 )
		return 1;
#endif
...
static void* UE1Win32LoadLibraryQuiet( const char* Filename )
{
	// UnterdrÃ¼ckt Windows-Dialoge wie "Einsprungpunkt nicht gefunden".
	// UE1 darf optionale/native Package-DLLs testen, ohne das Spiel zu unterbrechen.
