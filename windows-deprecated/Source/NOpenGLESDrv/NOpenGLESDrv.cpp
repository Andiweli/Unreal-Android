...
// UE1_WINDOWS_EFFECTFIX_V4_DIAGNOSTIC_BEGIN
static UBOOL GUE1GLESLoggedMaskedTranslucentV4 = false;
static DWORD GUE1GLESLoggedEffectUploadsV4 = 0;
// UE1_WINDOWS_EFFECTFIX_V4_DIAGNOSTIC_END
...
static BYTE UE1GLESPaletteEffectAlpha( const FTextureInfo& Info, BYTE Index, UBOOL Masked, UBOOL bMaineffect, UBOOL bSmokeBlack, UBOOL bHubWaterFall2, UBOOL bHubSmoke1, UBOOL bHubWaterRings2, BYTE WR2Corner0, BYTE WR2Corner1, BYTE WR2Corner2, BYTE WR2Corner3 )
{
	...
	if( bHubWaterFall2 )
	{
		// Die eigentliche Wasserfall-Textur nicht aggressiv wegmaskieren.
		// Sie bleibt regulär translucent.
		return 255;
	}

	if( bHubSmoke1 )
	{
		// Smoke1 ist die Gischt-/Mist-Overlay-Textur am Wasserfall.
		// Nur den wirklich schwarzen bzw. fast schwarzen Rand verwerfen,
		// die schwachen dunklen Nebelanteile aber erhalten.
		FColor C = Info.Palette[Index];
		INT MaxC = Max( C.R, Max( C.G, C.B ) );
		INT MinC = Min( C.R, Min( C.G, C.B ) );

		// Nur echte Key-/Randpixel wegwerfen.
		if( Index == 0 )
			return 0;

		// Fast schwarzer, nahezu neutraler Rand.
		if( MaxC <= 8 && ( MaxC - MinC ) <= 4 )
			return 0;
...
UBOOL UNOpenGLESRenderDevice::Init( UViewport* InViewport )
{
	...
	debugf( NAME_Log, "GLES2: EffectFixV4 active - Windows keeps PF_Masked for legacy translucent effects" );
...
void UNOpenGLESRenderDevice::SetBlend( DWORD PolyFlags, UBOOL InverseOrder )
{
	...
	{
		// UE1 Windows EffectFixV4:
		// Keep PF_Masked on translucent legacy effects. Removing it disables
		// SF_AlphaTest and makes explosion/smoke/splash billboards visible as
		// rectangular quads. Android already relies on this combination.
		if( ( PolyFlags & PF_Masked ) && !GUE1GLESLoggedMaskedTranslucentV4 )
		{
			GUE1GLESLoggedMaskedTranslucentV4 = true;
			debugf( NAME_Log, "GLES2: EffectFixV4 retained PF_Masked on PF_Translucent polygon" );
		}
	}
...
void UNOpenGLESRenderDevice::UploadTexture( FTextureInfo& Info, UBOOL Masked, UBOOL NewTexture, INT BaseMip )
{
	...
	DWORD EffectLogBit = bMaineffect ? 1 : ( bSmokeBlack ? 2 : ( bHubSmoke1 ? 4 : ( bHubWaterRings2 ? 8 : 0 ) ) );
	if( EffectLogBit && !( GUE1GLESLoggedEffectUploadsV4 & EffectLogBit ) )
	{
		GUE1GLESLoggedEffectUploadsV4 |= EffectLogBit;
		char EffectName[512];
		UE1GLESGetTextureObjectName( Info, EffectName, sizeof(EffectName) );
		debugf( NAME_Log, "GLES2: EffectFixV4 upload matched %s masked=%i", EffectName, Masked ? 1 : 0 );
	}
