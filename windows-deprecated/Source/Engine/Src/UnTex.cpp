...
#if defined(PLATFORM_WIN32) || defined(_WIN32) // UE1_WINDOWS_LOWDETAIL_GETINFO_V17_BEGIN
	// LOW texture detail is selected by UnrealVideoMenu through UClient::LowDetailTextures.
	// Do not destructively remove mip 0. Instead expose mip 1 as the base mip for this frame.
	if( Client && Client->LowDetailTextures && Mips.Num() > 1 && !( TextureFlags & ( TF_Realtime | TF_Parametric | TF_RealtimeChanged | TF_RealtimePalette ) ) )
	{
		TextureInfo.CacheID ^= (1ULL << 61);
		TextureInfo.UScale = Scale * 2.0f;
		TextureInfo.VScale = Scale * 2.0f;
		TextureInfo.USize = Mips(1).USize;
		TextureInfo.VSize = Mips(1).VSize;
		TextureInfo.UClamp = TextureInfo.USize;
		TextureInfo.VClamp = TextureInfo.VSize;
		TextureInfo.NumMips = Mips.Num() - 1;
		for( INT LowMipIndex = 1; LowMipIndex < Mips.Num(); LowMipIndex++ )
		{
			Mips(LowMipIndex).DataPtr = &Mips(LowMipIndex).DataArray(0);
			TextureInfo.Mips[LowMipIndex - 1] = &Mips(LowMipIndex);
		}
		for( INT ClearMipIndex = TextureInfo.NumMips; ClearMipIndex < MAX_MIPS; ClearMipIndex++ )
			TextureInfo.Mips[ClearMipIndex] = NULL;
		static UBOOL UE1WinLowDetailLogged = 0;
		if( !UE1WinLowDetailLogged )
		{
			UE1WinLowDetailLogged = 1;
			debugf( NAME_Log, "Engine: Windows LOW texture detail active through UTexture::GetInfo" );
		}
	}
#endif // UE1_WINDOWS_LOWDETAIL_GETINFO_V17_END
...
#if !( defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__) || defined(PLATFORM_WIN32) || defined(_WIN32) ) // UE1_WINDOWS_TEXTURE_DETAIL_V16
	if
	(	Ar.IsLoading()
	&&	!GIsEditor
	&&	Client 
