...
// UE1_WINDOWS_MULTIPLAYER_OPTIONS_MENU_PORT_V11_BEGIN
#ifndef UE1_WINDOWS_MENU_PATCHES
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__) || defined(PLATFORM_WIN32) || defined(_WIN32)
#define UE1_WINDOWS_MENU_PATCHES 1
#else
#define UE1_WINDOWS_MENU_PATCHES 0
#endif
#endif
// UE1_WINDOWS_MULTIPLAYER_OPTIONS_MENU_PORT_V11_END

// UE1_WINDOWS_RUNTIME_MENU_PORT_V9_BEGIN
#ifndef UE1_RUNTIME_MENU_PATCHES
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__) || defined(PLATFORM_WIN32) || defined(_WIN32)
#define UE1_RUNTIME_MENU_PATCHES 1
#else
#define UE1_RUNTIME_MENU_PATCHES 0
#endif
#endif
// UE1_WINDOWS_RUNTIME_MENU_PORT_V9_END
...
#if UE1_WINDOWS_MENU_PATCHES // UE1_WINDOWS_MULTIPLAYER_OPTIONS_MENU_PORT_V11
...

{
	...
	// UE1_WINDOWS_SKIP_AUDIOVIDEO_MENU_PATCH_V11_BEGIN
	...
	#endif
	// UE1_WINDOWS_SKIP_AUDIOVIDEO_MENU_PATCH_V11_END
...
// UE1_WINDOWS_START_STRONG_MOUSE_CAPTURE_V15_BEGIN
#if defined(PLATFORM_WIN32) || defined(_WIN32)
		
if( hWnd && !GIsEditor && !Temporary )
...
// UE1_WINDOWS_START_STRONG_MOUSE_CAPTURE_V15_END  		// UE1_WINDOWS_START_MOUSE_CAPTURE_V12_BEGIN #if defined(PLATFORM_WIN32) || defined(_WIN32) 		if( !GIsEditor && !Temporary ) 		{ 			SDL_RaiseWindow( hWnd ); 			SetMouseCapture( 1, 1, 0 ); 		} #endif 		// UE1_WINDOWS_START_MOUSE_CAPTURE_V12_END
...
if( hWnd != SDL_GetMouseFocus() && hWnd != SDL_GetKeyboardFocus() )
...
// UE1_WINDOWS_STRONG_MOUSE_CAPTURE_V15_BEGIN
#if defined(PLATFORM_WIN32) || defined(_WIN32)
...
SDL_WarpMouseInWindow( hWnd, SizeX / 2, SizeY / 2 );
...
// UE1_WINDOWS_STRONG_MOUSE_CAPTURE_V15_END
...
UBOOL UNSDLViewport::TickInput()
{
	...
	while( SDL_PollEvent( &Ev ) )
	{
		switch( Ev.type )
		{
			case SDL_QUIT:
				...
				QuitRequested = true;
				return true;
			case SDL_WINDOWEVENT: 						// UE1_WINDOWS_FOCUS_STRONG_MOUSE_CAPTURE_V15_BEGIN #if defined(PLATFORM_WIN32) || defined(_WIN32) 				if( hWnd && Ev.window.windowID == SDL_GetWindowID( hWnd ) ) 				{ 					if( Ev.window.event == SDL_WINDOWEVENT_FOCUS_GAINED || Ev.window.event == SDL_WINDOWEVENT_RESTORED || Ev.window.event == SDL_WINDOWEVENT_SHOWN ) 					{ 						SetMouseCapture( 1, 1, 0 ); 					} 					else if( Ev.window.event == SDL_WINDOWEVENT_FOCUS_LOST || Ev.window.event == SDL_WINDOWEVENT_MINIMIZED || Ev.window.event == SDL_WINDOWEVENT_HIDDEN ) 					{ 						SetMouseCapture( 0, 1, 0 ); 					} 				} #endif 			// UE1_WINDOWS_FOCUS_STRONG_MOUSE_CAPTURE_V15_END 				break; 			case SDL_TEXTINPUT:
#if PLATFORM_ANDROID
	...
	// UE1_WINDOWS_MOUSE_CAPTURE_WATCHDOG_V15_BEGIN #if defined(PLATFORM_WIN32) || defined(_WIN32) 	if( hWnd && SDL_GetKeyboardFocus() == hWnd && !SDL_GetRelativeMouseMode() ) 	{ 		SetMouseCapture( 1, 1, 0 ); 	} #endif 	// UE1_WINDOWS_MOUSE_CAPTURE_WATCHDOG_V15_END 	// Constantly hammer the input system with axis events for axes that are not zero.
