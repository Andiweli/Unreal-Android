...
void UObject::execStructCmpEq( FFrame& Stack, BYTE*& Result )
{
	...
	// Get struct (fixed-width GObjects index).
...
void UObject::CallFunction( FFrame& Stack, BYTE*& Result, UFunction* Function )
{
	...
	else
	{
		...
		NewStack.Code++;
		// The EX_BeginFunction size bytes are stale 32-bit metadata. Consume them
		// only to advance bytecode; use relinked properties for native size/offset.
