...
static void UE1AndroidDrawVersionAbiOverlay( UViewport* Viewport )
{
	...
	if( !UE1AndroidIsRootMainMenu(Viewport) || !Viewport->Canvas )
		return;
	...
	UFont* Font = Canvas->SmallFont ? Canvas->SmallFont : Canvas->MedFont;
	if( !Font || Canvas->X<=0 || Canvas->Y<=0 )
		return;

	// Unreal's menu help panel leaves a local origin and a 228x128 clip active.
	// Native console rendering happens afterwards, so drawing with that state puts
	// the version below the visible screen. Temporarily restore the full logical
	// canvas, draw in white, then restore every changed field.
	...
	const FLOAT SavedZ = Canvas->Z;
	const FColor SavedColor = Canvas->Color;
	const BYTE SavedStyle = Canvas->Style;
	const DWORD SavedCenter = Canvas->bCenter;
	const DWORD SavedNoSmooth = Canvas->bNoSmooth;

	Canvas->OrgX = 0.0f;
	Canvas->OrgY = 0.0f;
	Canvas->ClipX = (FLOAT)Canvas->X;
	Canvas->ClipY = (FLOAT)Canvas->Y;
	Canvas->CurX = 0.0f;
	Canvas->CurY = 0.0f;
	Canvas->CurYL = 0.0f;
	Canvas->Z = 1.0f;
	...
	INT XL=0, YL=0;
	Canvas->StrLen( Font, XL, YL, Text );
	const INT X = Max<INT>( 4, Canvas->X - XL - 8 );
	const INT Y = Max<INT>( 4, Canvas->Y - YL - 6 );
	Canvas->Printf( Font, X, Y, "%s", Text );
