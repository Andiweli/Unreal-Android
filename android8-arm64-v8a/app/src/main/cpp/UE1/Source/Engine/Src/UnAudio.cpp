...
void USound::PostLoad()
{
	...
	Super::PostLoad();
#if PLATFORM_64BIT
	// arm64 migration safety: if a sound package was loaded before the audio
	// subsystem assigned USound::Audio, Serialize() could not register the
	// OpenAL buffer.  Register it here as soon as both Audio and Data exist.
	if( Audio && !GIsEditor && !Handle && Data.Num() > 0 )
		Audio->RegisterSound( this );
#endif
	unguard;
};
