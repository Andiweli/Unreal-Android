...
#if PLATFORM_64BIT
static inline UBOOL UE1Android64TickBadPtr( const void* Ptr )
{
	const UPTRINT Value = (UPTRINT)Ptr;
	return !Ptr || Value < 0x10000 || ((Value & (sizeof(void*)-1)) != 0);
}

static INT GUE1Android64TickWarnCount = 0;
static void UE1Android64TickWarn( const char* What, AActor* Actor, const void* Ptr )
{
	if( GUE1Android64TickWarnCount++ < 64 )
	{
		const char* ActorName = "<null>";
		const char* ClassName = "<null>";
		if( Actor && !UE1Android64TickBadPtr(Actor) )
		{
			ActorName = Actor->GetName();
			ClassName = Actor->GetClassName();
		}
		debugf( NAME_Warning, "ANDROID64 Tick safety: %s actor=%s class=%s ptr=%p", What, ActorName, ClassName, Ptr );
	}
}

static inline UBOOL UE1Android64TickInvalidMainFrame( AActor* Actor )
{
	FMainFrame* Frame = Actor ? Actor->GetMainFrame() : NULL;
	if( UE1Android64TickBadPtr(Frame) )
	{
		UE1Android64TickWarn( "bad MainFrame - skipping script state/timer sensitive work", Actor, Frame );
		return 1;
	}
	return 0;
}

static inline UBOOL UE1Android64TickActorGone( AActor* Actor )
{
	return !Actor || Actor->bDeleteMe || !Actor->XLevel || !Actor->Level;
}
#endif
...
UBOOL AActor::Tick( FLOAT DeltaSeconds, ELevelTick TickType )
{
...
#if PLATFORM_64BIT
	// arm64 migration safety: sporadic gameplay crashes have shown up as
	// AActor::Tick() faulting on very small/null sub-pointers after actors were
	// destroyed or partially restored from old 32-bit state data.  Keep this local
	// to Tick; do not globally hide bad UObject references.
	if( bDeleteMe || !XLevel || !Level )
		return 1;
	if( Owner && (((UPTRINT)Owner < 0x10000) || (((UPTRINT)Owner & (sizeof(void*)-1)) != 0)) )
		Owner = NULL;
	if( Base && (((UPTRINT)Base < 0x10000) || (((UPTRINT)Base & (sizeof(void*)-1)) != 0)) )
		Base = NULL;
	if( Mesh && (((UPTRINT)Mesh < 0x10000) || (((UPTRINT)Mesh & (sizeof(void*)-1)) != 0)) )
		Mesh = NULL;
	const UBOOL bAndroid64BadMainFrame = UE1Android64TickInvalidMainFrame( this );
#else
	const UBOOL bAndroid64BadMainFrame = 0;
#endif
...
#if PLATFORM_64BIT
		&& XLevel->Model && XLevel->Model->Nodes
...
#if PLATFORM_64BIT
	if( Owner && (UE1Android64TickBadPtr(Owner) || !Owner->IsValid()) )
	{
		UE1Android64TickWarn( "clearing invalid Owner before owner-first tick", this, Owner );
		Owner = NULL;
	}
#endif
	...
	&&	(++Iterations <= 4) )
	{
		...
		if( AnimFrame >= 0.0 )
		{
			...
			{
#if PLATFORM_64BIT
				if( UE1Android64TickBadPtr(Mesh) || !Mesh->IsValid() )
				{
					UE1Android64TickWarn( "clearing invalid Mesh before anim notify", this, Mesh );
					Mesh = NULL;
				}
#endif
			}
...
#if PLATFORM_64BIT
							if( UE1Android64TickActorGone(this) )
								return 1;
#endif
						}
						continue;
			...
			else if( bAnimLoop )
			{
				...
				{
					if( !bAndroid64BadMainFrame && GetMainFrame()->LatentAction == EPOLL_FinishAnim )
						bAnimFinished = 1;
					if ( !bSimulatedPawn )
					{
						eventAnimEnd();
...
#if PLATFORM_64BIT
			if( UE1Android64TickBadPtr(PlayerPawn->Player) )
			{
				UE1Android64TickWarn( "invalid Player pointer before PlayerTick", this, PlayerPawn->Player );
				return 1;
			}
#endif
...
#if PLATFORM_64BIT
			if( UE1Android64TickActorGone(this) || UE1Android64TickBadPtr(PlayerPawn->Player) )
				return 1;
#endif
...
#if PLATFORM_64BIT
		if( Base && (UE1Android64TickBadPtr(Base) || !Base->IsValid()) )
		{
			UE1Android64TickWarn( "clearing invalid Base before mover follow", this, Base );
			Base = NULL;
		}
#endif
	...
	&&	(!NetDriver || !NetDriver->ServerConnection || NetDriver->ServerConnection->State==USOCK_Open) )
	{
		...
		for( INT iActor=iFirstDynamicActor; iActor<Num(); iActor++ )
		{
			...
			&& !Actor->bDeleteMe
				...
				Updated += Actor->Tick(DeltaSeconds,TickType);
		}
