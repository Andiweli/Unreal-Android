...
class CORE_API UClass : public UState
{
	...
	// UClass interface.
#if PLATFORM_64BIT
	INT GetNativePropertiesSize() const
	{
		return NativePropertiesSize;
	}
	void SetNativePropertiesSize( INT NewSize );
	void ForceNativePropertiesSize( INT NewSize );
	void EnsureNativePropertiesSize();
	INT FindRegisteredNativePropertiesSize() const;
	static void RegisterNativePropertiesSize( FName PackageName, FName ClassName, INT NativeSize );
#endif
