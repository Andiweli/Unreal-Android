...
#if PLATFORM_64BIT
// Cheap arm64 script pointer sanity.  This is intentionally only an address
// shape check; it avoids dereferencing tiny or unaligned garbage operands while
// keeping the hot script paths free from the broad GObj scans that previously
// made menus and controls sluggish.
static inline UBOOL UE1AndroidScriptLooksLikeUsablePointer64( const void* Ptr )
{
...
void UObject::execLocalVariable( FFrame& Stack, BYTE*& Result )
{
	...
	GProperty = ((UProperty*)Stack.ReadPtr());
...
#if PLATFORM_64BIT
	if( ClassContext && !UE1AndroidScriptLooksLikeUsablePointer64(ClassContext) )
		ClassContext = NULL;
#endif
	if( ClassContext )
	{
...
{
	Result = Stack.Locals + ((UProperty*)Stack.ReadPtr())->Offset;
}
...
#if PLATFORM_64BIT
	if( NewContext && !UE1AndroidScriptLooksLikeUsablePointer64(NewContext) )
		NewContext = NULL;
#endif
	if( NewContext != NULL )
	{
...
void UObject::execFinalFunction( FFrame& Stack, BYTE*& Result )
{
	...
	CallFunction( Stack, Result, (UFunction*)Stack.ReadPtr() );
...
void UObject::execStructMember( FFrame& Stack, BYTE*& Result )
{
	...
	UProperty* Property = (UProperty*)Stack.ReadPtr();
...
void UObject::execObjectConst( FFrame& Stack, BYTE*& Result )
{
	...
	*(UObject**)Result = (UObject*)Stack.ReadPtr();
	unguardexecSlow;
}
...
void UObject::execDynamicCast( FFrame& Stack, BYTE*& Result )
{
	...
	UClass* Class = (UClass *)Stack.ReadPtr();
...
#if PLATFORM_64BIT
	if( Castee && (!UE1AndroidScriptLooksLikeUsablePointer64(Castee) || !UE1AndroidScriptLooksLikeUsablePointer64(Class)) )
		Castee = NULL;
#endif
...
void UObject::execMetaCast( FFrame& Stack, BYTE*& Result )
{
	...
	// Get destination class of dynamic actor class.
	UClass* MetaClass = (UClass*)Stack.ReadPtr();
	...
	UObject* Castee = *(UObject**)Addr;
#if PLATFORM_64BIT
	if( Castee && (!UE1AndroidScriptLooksLikeUsablePointer64(Castee) || !UE1AndroidScriptLooksLikeUsablePointer64(MetaClass)) )
		Castee = NULL;
#endif
...
#endif
	Stack.Step( Stack.Object, Addr );
	UObject* Obj = *(UObject**)Addr;
#if PLATFORM_64BIT
	if( Obj && !UE1AndroidScriptLooksLikeUsablePointer64(Obj) )
		Obj = NULL;
#endif
...
void UObject::execObjectToString( FFrame& Stack, BYTE*& Result )
{
	...
	UObject* Obj = *(UObject**)Addr;
#if PLATFORM_64BIT
	// Legacy UnrealScript often stringifies optional object refs while menu/game
	// scripts are transitioning maps.  On arm64 a still-misaligned or stale
	// script object slot must not be allowed to walk into FName::operator*().
	// Keep this guard local to Object->String conversion; do not globally hide
	// VM/object errors and do not touch menu/input/resolution paths.
	const char* SafeName = "None";
	if( Obj && ((UPTRINT)Obj >= 0x10000) && (((UPTRINT)Obj & (sizeof(void*)-1)) == 0) )
	{
		FName ObjName = Obj->GetFName();
		if( ObjName.IsValid() )
			SafeName = *ObjName;
	}
	appStrcpy((char*)Result, SafeName);
#else
	appStrcpy((char*)Result, Obj ? Obj->GetName() : "None");
#endif
	unguardexecSlow;
}
...
#endif
	Stack.Step( Stack.Object, Addr );
	FName Name = *(FName*)Addr;
	*(DWORD*)Result = Name!=NAME_None ? 1 : 0;
	unguardexecSlow;
}
...
#if PLATFORM_64BIT
	// Multiplayer/projectile scripts can reach ClassIsChildOf through short
	// boolean chains while actors are being destroyed.  A stale 32-bit-sized
	// class slot must not be dereferenced by UStruct::IsChildOf().  Keep this
	// local to ClassIsChildOf; do not add a broad VM softfail or object scan.
	if( K && !UE1AndroidScriptLooksLikeUsablePointer64(K) )
		K = NULL;
	if( C && !UE1AndroidScriptLooksLikeUsablePointer64(C) )
		C = NULL;
#endif
