...
#if PLATFORM_64BIT
	// If a null EX_Context only writes the legacy 4-byte zero-fill for an object
	// expression, the upper half of this temporary must still be deterministic.
	// Otherwise object/class locals can later become tiny garbage pointers that
	// break Spawn(), Trigger/Timer state code and CleanupDestroyed().
