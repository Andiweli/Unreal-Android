...
#if PLATFORM_64BIT
static inline INT UE1AndroidObjectPropertyAlignment64( UStruct* ParentStruct, const char* PropertyName, DWORD PropertyFlags )
{
	if( !ParentStruct )
		return sizeof(UObject*);

	const char* ParentName = ParentStruct->GetName();

	// UFunction parameter frames for generated UE1 C++ wrappers come from
	// EngineClasses.h, which is emitted under #pragma pack(push,4).  The
	// parameters of intrinsic/native/event thunks therefore must remain
	// pack(4)-compatible, or calls like ClientHearSound(Actor,Id,Sound,...)
	// read the second object pointer from the wrong offset.
	//
	// Locals inside script functions are different: they are VM-owned runtime
	// storage, not a generated C++ wrapper struct.  Actor/object locals used by
	// iterators and timer/event scripts must use native pointer alignment/size
	// on arm64, otherwise VisibleCollidingActors/IsA-style code receives tiny
	// bogus contexts such as 0x1d12 and gameplay triggers silently stop.
	if( ParentStruct->IsA(UFunction::StaticClass) )
	{
		UFunction* Function = (UFunction*)ParentStruct;
		if( PropertyFlags & CPF_Parm )
		{
			if( Function->FunctionFlags & (0x0400/*FUNC_Intrinsic*/|0x0800/*FUNC_Event*/) )
				return 4;
		}
		return sizeof(UObject*);
	}

	// Hand-written native classes contain real 64-bit pointers and need native
	// object-reference alignment. Generated EngineClasses-style classes are
	// still emitted under pack(4), so they must not be globally widened.
	if(	appStricmp(ParentName,"Bitmap")==0
	||	appStricmp(ParentName,"WetTexture")==0
	||	appStricmp(ParentName,"IceTexture")==0
	||	appStricmp(ParentName,"Canvas")==0
	||	appStricmp(ParentName,"Player")==0
	||	appStricmp(ParentName,"Viewport")==0
	||	appStricmp(ParentName,"Console")==0 )
		return sizeof(UObject*);

	if( appStricmp(ParentName,"Texture")==0 )
		return (PropertyName && appStricmp(PropertyName,"BumpMap")==0) ? 4 : sizeof(UObject*);

	if( !(ParentStruct->GetFlags() & RF_Intrinsic) )
		return sizeof(UObject*);

	return 4;
}

static inline UBOOL UE1AndroidIsDynamicArrayStruct64( UStruct* Struct )
{
	return Struct && appStricmp(Struct->GetName(),"DynamicArray")==0;
}
#endif
...
class CORE_API UObjectProperty : public UProperty
{
	...
	{
#if PLATFORM_64BIT
		Offset = Align( GetParentStruct()->PropertiesSize, UE1AndroidObjectPropertyAlignment64(GetParentStruct(),GetName(),PropertyFlags) );
#else
...
class CORE_API UStructProperty : public UProperty
{
	...
	{
#if PLATFORM_64BIT
		if( UE1AndroidIsDynamicArrayStruct64(Struct) )
