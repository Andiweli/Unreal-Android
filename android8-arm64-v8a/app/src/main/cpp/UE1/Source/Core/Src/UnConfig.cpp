...
#if defined(PLATFORM_ANDROID) || defined(UNREAL_ANDROID) || defined(__ANDROID__)
	// Do not synchronously rewrite the ini file for every single SetString().
	// UObject::SaveConfig() can touch dozens of properties, and gameplay scripts
	// may call SaveConfig() from pickup/bump/state paths.  The old Android
	// autosave wrote Unreal.ini once per property, causing long game-thread stalls
	// where only the SDL/menu pump still appeared responsive.  Keep persistence
	// through explicit SaveAllConfigs() calls and shutdown flushes instead.
	if( Result && Cfg && !GUE1AndroidConfigAutosaveGuard )
	{
		static INT DeferredLogCount = 0;
		if( DeferredLogCount++ < 12 )
			__android_log_print( ANDROID_LOG_INFO, "UE1Config", "Deferred config touch: %s", Cfg->Filename );
	}
#endif
...
{
#if defined(PLATFORM_ANDROID) || defined(__ANDROID__)
	static INT AndroidSaveAllLogCount = 0;
	if( AndroidSaveAllLogCount++ < 32 )
