...
#if PLATFORM_64BIT
	// arm64 safety: reject tiny/unaligned garbage pointers before touching
	// UObject::Index.  For normal live objects, use the original fast index-table
	// validation; the earlier full GObj.Objects scan was safe but made level and
	// save loading noticeably slower on Android.
	UPTRINT ThisValue = (UPTRINT)this;
	if( ThisValue < 0x10000 || (ThisValue & (sizeof(void*)-1)) != 0 )
	{
		static UBOOL WarnedBadObjectPointer = 0;
		if( !WarnedBadObjectPointer )
		{
			WarnedBadObjectPointer = 1;
			debugf( NAME_Warning, "Invalid object pointer %p", this );
		}
		return 0;
	}
	INT ObjectIndex = Index;
	if( !GObj.Objects.IsValidIndex(ObjectIndex) )
	{
		debugf( NAME_Warning, "Invalid object index %i", ObjectIndex );
		return 0;
	}
	if( GObj.Objects(ObjectIndex)!=this )
	{
		debugf( NAME_Warning, "Unlisted object pointer %p index=%i", this, ObjectIndex );
		return 0;
	}
	return 1;
#else
...
#if PLATFORM_64BIT
			// Save arm64 execution stacks in legacy script-offset space, then remap
			// back to native offsets on load.  Saves written by the 64-bit build used
			// expanded bytecode positions; remapping those a second time makes resume
			// and latent state loading unreliable.
			if( Ar.IsSaving() && Offset!=INDEX_NONE )
				Offset = UE1AndroidUnmapScriptOffset64( MainFrame->Node, Offset );
#endif
			Ar << AR_INDEX(Offset);
#if PLATFORM_64BIT
			if( Ar.IsLoading() && Offset!=INDEX_NONE )
				Offset = UE1AndroidRemapScriptOffset64( MainFrame->Node, Offset );
#endif
...
void FObjectManager::PreRegister( UObject* InObject, FName InPackageName )
{
	...
	else
	{
		...
		InObject->LinkerIndex = (PTRINT)AutoRegister;
...
void FObjectManager::Init()
{
	...
	{
		Next = (UObject*)(UPTRINT)Object->LinkerIndex;
...
)
{
	...
	DWORD ClassFlags = 0;
#if PLATFORM_64BIT
	INT NativePropertiesSize64 = 0;
#endif
#if PLATFORM_64BIT
	if( InClass!=UClass::StaticClass )
		InClass->EnsureNativePropertiesSize();
#endif
	if( !Obj )
	{
		// Create a new object.
		INT AllocationSize = InClass!=UClass::StaticClass ? InClass->GetPropertiesSize() : sizeof(UClass);
#if PLATFORM_64BIT
		if( AllocationSize < (INT)sizeof(UObject) )
			AllocationSize = sizeof(UObject);
#endif
		Obj = (UObject *)appMalloc( AllocationSize, *InName );
	}
	else
	{
...
#if PLATFORM_64BIT
			NativePropertiesSize64 = ((UClass*)Obj)->GetNativePropertiesSize();
			if( NativePropertiesSize64 <= 0 )
				NativePropertiesSize64 = ((UClass*)Obj)->GetPropertiesSize();
#endif
		}
		Index = Obj->Index;
		Obj->~UObject();
...
#if PLATFORM_64BIT
		if( NativePropertiesSize64 > 0 )
			((UClass*)Obj)->SetNativePropertiesSize( NativePropertiesSize64 );
#endif
	}
