...
UBOOL UNOpenALAudioSubsystem::PlaySound( AActor* Actor, INT Id, USound* Sound, FVector Location, FLOAT Volume, FLOAT Radius, FLOAT Pitch )
{
...
#if PLATFORM_64BIT
	// Defensive arm64 guard: native script/event parameter bugs often show up as
	// obviously unaligned UObject pointers (for example low bits from an INT slot
	// mixed into the following pointer).  Never dereference those in the audio
	// backend; reject them before touching Sound->Handle so a bad menu sound
	// parameter cannot take the whole renderer down.  Correctly allocated UE1
	// UObjects are at least pointer-aligned.
	if( ((UPTRINT)Sound & (sizeof(void*)-1)) != 0 )
	{
		static UBOOL WarnedBadSoundPointer = 0;
		if( !WarnedBadSoundPointer )
		{
			WarnedBadSoundPointer = 1;
			debugf( NAME_Warning, "arm64 PlaySound rejected unaligned USound pointer %p id=%i", Sound, Id );
		}
		return false;
	}
#endif

	if( !Sound->Handle && Sound->Data.Num() > 0 )
	{
		// Some UE1 packages can deserialize sounds before the Android/OpenAL
		// subsystem is ready.  In 32-bit this still usually worked because the
		// load order happened to register them early; on arm64 we must also
		// allow late registration at first real playback.
		RegisterSound( Sound );
	}
	if( !Sound->Handle )
	{
#if PLATFORM_64BIT
		static UBOOL WarnedMissingSoundHandle = 0;
		if( !WarnedMissingSoundHandle )
		{
			WarnedMissingSoundHandle = 1;
			debugf( NAME_Warning, "arm64 PlaySound skipped sound without OpenAL buffer: %s data=%i", Sound ? Sound->GetName() : "NULL", Sound ? Sound->Data.Num() : 0 );
		}
#endif
		return false;
	}
	...
	if( !alIsBuffer( Buf ) )
	{
		...
		if( Sound->Data.Num() > 0 )
		{
			Sound->Handle = NULL;
			RegisterSound( Sound );
			Buf = (ALuint)(UPTRINT)Sound->Handle;
		}
#endif
		if( !Sound->Handle || !alIsBuffer( Buf ) )
			return false;
	}
