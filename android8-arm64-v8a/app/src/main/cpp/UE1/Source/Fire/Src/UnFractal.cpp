...
#if PLATFORM_64BIT
static void UE1AndroidEnsureFireNativeClassSizes()
{
	guard(UE1AndroidEnsureFireNativeClassSizes);

	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("FractalTexture"), sizeof(UFractalTexture) );
	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("FireTexture"),    sizeof(UFireTexture) );
	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("WaterTexture"),   sizeof(UWaterTexture) );
	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("WaveTexture"),    sizeof(UWaveTexture) );
	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("WetTexture"),     sizeof(UWetTexture) );
	UClass::RegisterNativePropertiesSize( FName("Fire"), FName("IceTexture"),     sizeof(UIceTexture) );

	if( UFractalTexture::StaticClass ) UFractalTexture::StaticClass->ForceNativePropertiesSize( sizeof(UFractalTexture) );
	if( UFireTexture::StaticClass    ) UFireTexture::StaticClass->ForceNativePropertiesSize( sizeof(UFireTexture) );
	if( UWaterTexture::StaticClass   ) UWaterTexture::StaticClass->ForceNativePropertiesSize( sizeof(UWaterTexture) );
	if( UWaveTexture::StaticClass    ) UWaveTexture::StaticClass->ForceNativePropertiesSize( sizeof(UWaveTexture) );
	if( UWetTexture::StaticClass     ) UWetTexture::StaticClass->ForceNativePropertiesSize( sizeof(UWetTexture) );
	if( UIceTexture::StaticClass     ) UIceTexture::StaticClass->ForceNativePropertiesSize( sizeof(UIceTexture) );
...
{
#if PLATFORM_64BIT
	UE1AndroidEnsureFireNativeClassSizes();
#endif
	// Call base.
	UTexture::PostLoad();
...
void UFireTexture::PostLoad()
{
	...
	UE1AndroidEnsureFireNativeClassSizes();
	if( GetClass() )
		GetClass()->EnsureNativePropertiesSize();
#endif
#if PLATFORM_64BIT
	if( sizeof(UFireTexture) != UFireTexture::StaticClass->GetPropertiesSize() )
		appErrorf( "arm64 native class size mismatch: FireTexture sizeof=%i props=%i native=%i", (INT)sizeof(UFireTexture), UFireTexture::StaticClass->GetPropertiesSize(), UFireTexture::StaticClass->GetNativePropertiesSize() );
	if( sizeof(UWetTexture) != UWetTexture::StaticClass->GetPropertiesSize() )
		appErrorf( "arm64 native class size mismatch: WetTexture sizeof=%i props=%i native=%i", (INT)sizeof(UWetTexture), UWetTexture::StaticClass->GetPropertiesSize(), UWetTexture::StaticClass->GetNativePropertiesSize() );
	if( sizeof(UWaveTexture) != UWaveTexture::StaticClass->GetPropertiesSize() )
		appErrorf( "arm64 native class size mismatch: WaveTexture sizeof=%i props=%i native=%i", (INT)sizeof(UWaveTexture), UWaveTexture::StaticClass->GetPropertiesSize(), UWaveTexture::StaticClass->GetNativePropertiesSize() );
	if( sizeof(UFractalTexture) != UFractalTexture::StaticClass->GetPropertiesSize() )
		appErrorf( "arm64 native class size mismatch: FractalTexture sizeof=%i props=%i native=%i", (INT)sizeof(UFractalTexture), UFractalTexture::StaticClass->GetPropertiesSize(), UFractalTexture::StaticClass->GetNativePropertiesSize() );
#else
