...
/* OUYA / Android API16 legacy build:
   SDL's Java controller manager may reference Android haptic entry points even when
   the Android haptic backend is not built. Provide weak no-op stubs to satisfy linking. */
__attribute__((weak)) int Android_AddHaptic(int device_id, const char *name)
{
    (void)device_id;
    (void)name;
    return -1;
...
__attribute__((weak)) int Android_RemoveHaptic(int device_id)
{
    (void)device_id;
    return -1;
}
