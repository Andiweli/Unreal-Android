...
,	Line( 0 )
{}
// IntrinsicSize is deliberately not initialized here. Like UClass::Constructor,
// it must survive in-place reconstruction of a registered intrinsic class.
...
// Script mirrors encode hidden pointer/TArray/FString members as 32-bit stand-ins.
// Widen only those placeholders on 64-bit while preserving 32-bit package offsets.
enum { MIRROR_PTR=-1, MIRROR_ARRAY=-2 };
static INT GetPlaceholderSize( UStruct* Owner, UProperty* Property )
{
	...
	{
		{ "Object",       "ObjectInternal",    MIRROR_PTR   },
		{ "Palette",      "Colors",            MIRROR_ARRAY },
		{ "Texture",      "Mips",              MIRROR_ARRAY },
		{ "FireTexture",  "Sparks",            MIRROR_ARRAY },
		{ "WaterTexture", "SourceFields",      MIRROR_PTR   },
		{ "WetTexture",   "LocalSourceBitmap", MIRROR_PTR   },
		{ "Canvas",       "FramePtr",          MIRROR_PTR   },
		{ "Console",      "vtblOut",           MIRROR_PTR   },
		{ "Player",       "vfOut",             MIRROR_PTR   },
		{ "Player",       "TravelItems",       MIRROR_ARRAY },
	};
...
EExprToken UStruct::SerializeExpr( INT& iCode, FArchive& Ar )
{
	...
	// Keep object operands at their original four-byte width. Package jump offsets
	// remain valid; native pointers are resolved through the global object table.
	#define XFER_OBJ(T) \
	{ \
		if( Ar.IsLoading() ) \
		{ \
		...
		{ \
			INT Idx = *(INT*)&Script(iCode); \
			T* Obj = Idx!=INDEX_NONE ? (T*)GObj.GetIndexedObject(Idx) : NULL; \
			Ar << (UObject*&)Obj; \
		} \
		iCode += sizeof(INT); \
	}
...
void UFunction::Serialize( FArchive& Ar )
{
	...
	// Serialized frame metadata was produced with 32-bit pointer sizes.
	if( Ar.IsLoading() )
	{
