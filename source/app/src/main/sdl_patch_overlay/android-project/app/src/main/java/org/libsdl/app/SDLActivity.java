...
*/
    ...
    public static boolean handleKeyEvent(View v, int keyCode, KeyEvent event, InputConnection ic) {
        ChromeOSInputLogger.logKey("SDLActivity handleKeyEvent IN", event); // UNREAL_ANDROID_CHROMEOS_INPUT_FILELOG_DIAG1
...
//        }
        ...
        boolean diagIsJoystickDevice = SDLControllerManager.isDeviceSDLJoystick(deviceId);
        ChromeOSInputLogger.log("SDLActivity handleKeyEvent classify", "deviceId=" + deviceId
                + " source=0x" + Integer.toHexString(source)
                + " isDeviceSDLJoystick=" + diagIsJoystickDevice);
        if (diagIsJoystickDevice) {
            // Note that we process events with specific key codes here
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                int padResult = SDLControllerManager.onNativePadDown(deviceId, keyCode);
                ChromeOSInputLogger.log("SDLActivity handleKeyEvent pad", "DOWN keyCode=" + keyCode + " result=" + padResult);
                if (padResult == 0) {
                    return true;
                }
            } else if (event.getAction() == KeyEvent.ACTION_UP) {
                int padResult = SDLControllerManager.onNativePadUp(deviceId, keyCode);
                ChromeOSInputLogger.log("SDLActivity handleKeyEvent pad", "UP keyCode=" + keyCode + " result=" + padResult);
                if (padResult == 0) {
                    return true;
                }
            }
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            ...
            ChromeOSInputLogger.log("SDLActivity handleKeyEvent keyboard", "onNativeKeyDown keyCode=" + keyCode);
            onNativeKeyDown(keyCode);
            return true;
        } else if (event.getAction() == KeyEvent.ACTION_UP) {
            ChromeOSInputLogger.log("SDLActivity handleKeyEvent keyboard", "onNativeKeyUp keyCode=" + keyCode);
