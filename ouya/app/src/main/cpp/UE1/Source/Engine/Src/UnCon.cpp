...
void UConsole::PostRender( FSceneNode* Frame )
{
	...
	*Frame = SavedFrame;
	...
	if( BigMessage[0] )
	{
		...
		const FLOAT CenterX = (Frame->X - XL) * 0.5f; // UNREAL_ANDROID_OUYA_CENTER_LOADING_TEXT
		const FLOAT CenterY = (Frame->Y - YL) * 0.5f;
		Viewport->Canvas->Printf( LargeFont, CenterX, CenterY, "%s", BigMessage );
	}
