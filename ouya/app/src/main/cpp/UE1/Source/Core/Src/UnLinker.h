	...
	void VerifyImport( INT i )
	{
		...
		&&	(Ver()>=50 || Import._ObjectPackage!=NAME_None) )
		{
			...
			if( Import.SourceIndex==-1 && (Ver()<50 || Pkg!=NULL) )
			{
				...
				// Android/OUYA runtime maps can contain editor-only import chains such as
				// Editor.TransBuffer. editor.u is intentionally not shipped with the game
				// data; failing this import would abort level transitions after maps that
				// still reference the editor transaction buffer.
				UBOOL bAndroidEditorOnlyImport = 0;
				if( !GIsEditor )
				{
					if( Pkg && appStricmp( Pkg->GetName(), "Editor" ) == 0 )
						bAndroidEditorOnlyImport = 1;
					if( appStricmp( *Import.ObjectName, "TransBuffer" ) == 0 )
						bAndroidEditorOnlyImport = 1;
					if( Import.SourceLinker && Import.SourceLinker->Filename )
					{
						char AndroidEditorFilename[1024];
						appStrncpy( AndroidEditorFilename, Import.SourceLinker->Filename, ARRAY_COUNT(AndroidEditorFilename) );
						appStrlwr( AndroidEditorFilename );
						if( appStrstr( AndroidEditorFilename, "editor.u" ) )
							bAndroidEditorOnlyImport = 1;
					}
				}
				if( bAndroidEditorOnlyImport )
				{
					debugf( NAME_Warning, "Ignoring editor-only import while loading runtime map on Android/OUYA: %s.%s", Pkg ? Pkg->GetName() : "<no package>", *Import.ObjectName );
					SafeReplace = 1;
				}
#endif
